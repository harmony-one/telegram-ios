...
readonly bazel_cmd=(
  env
  'PATH=/bin:/usr/bin'
  "/Users/nageshkumarmishra/Telegram-iOS/build-input/bazel-6.4.0-darwin-x86_64"
...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@rules_xcodeproj_generated//generator/Telegram/Telegram_xcodeproj:Telegram_xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids")
